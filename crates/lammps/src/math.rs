use crate::geomutil_util::Float;
use std::iter;

pub fn range<T: Float>(begin: T, end: T, count: usize) -> impl Iterator<Item = T> {
    let n = count.checked_sub(1).unwrap_or_default().max(1);
    let step = (end - begin) / T::from(n as f64);
    (0..count).map(move |i| T::from(i as f64).mul_add(step, begin))
}

pub trait IteratorAvg<T>: Iterator<Item = T> {
    fn avg(self) -> Option<T>;
    fn avg_with_std(self) -> Option<(T, T)>;
    fn std(self) -> Option<T>;
}

impl<I, T: Float> IteratorAvg<T> for I
where
    I: Iterator<Item = T>,
{
    fn avg(self) -> Option<T> {
        iter::zip(self, 1usize..)
            .reduce(|(sum, _), (next, cnt)| (sum + next, cnt))
            .map(|(sum, cnt)| sum / T::from(cnt as f64))
    }

    fn avg_with_std(self) -> Option<(T, T)> {
        let values = self.collect::<Vec<_>>();
        let avg = values.iter().copied().avg()?;
        values
            .into_iter()
            .map(|x| (x - avg).powi(2))
            .avg()
            .map(|std| (avg, std.sqrt()))
    }

    fn std(self) -> Option<T> {
        self.avg_with_std().map(|(_, std)| std)
    }
}

#[cfg(test)]
mod tests {
    use super::*;
    use assert_float_eq::assert_f32_near;
    use std::iter;

    #[test]
    fn test_iterator_avg() {
        let data: Vec<f32> = vec![];
        let avg = data.iter().copied().avg();
        let std = data.iter().copied().std();
        assert!(avg.is_none());
        assert!(std.is_none());
        let data: Vec<f32> = vec![5.0];
        let avg = data.iter().copied().avg();
        let std = data.iter().copied().std();
        assert!(avg.is_some());
        assert_f32_near!(avg.unwrap(), 5.0, 6);
        assert!(std.is_some());
        assert_f32_near!(std.unwrap(), 0.0, 6);
        let data: Vec<f32> = vec![1.0, 2.0, 3.0];
        let avg = data.iter().copied().avg();
        let std = data.iter().copied().std();
        assert!(avg.is_some());
        assert_f32_near!(avg.unwrap(), 2.0, 6);
        assert!(std.is_some());
        assert_f32_near!(std.unwrap(), 0.81649658, 6);
    }

    #[test]
    fn test_range() {
        let collected = range(0.0, 10.0, 5).collect::<Vec<_>>();
        let values = [0.0, 2.5, 5.0, 7.5, 10.0];
        assert_eq!(collected.len(), values.len());
        iter::zip(collected, values).for_each(|(got, want)| {
            assert_f32_near!(got, want, 6);
        });
        assert_eq!(range(4.0, 5.0, 0).count(), 0);
        let collected = range(4.0, 5.0, 1).collect::<Vec<_>>();
        let values = [4.0];
        assert_eq!(collected.len(), values.len());
        iter::zip(collected, values).for_each(|(got, want)| {
            assert_f32_near!(got, want, 6);
        });
        let collected = range(4.0, 5.0, 2).collect::<Vec<_>>();
        let values = [4.0, 5.0];
        assert_eq!(collected.len(), values.len());
        iter::zip(collected, values).for_each(|(got, want)| {
            assert_f32_near!(got, want, 6);
        });
    }
}
